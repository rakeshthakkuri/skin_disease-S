...
/**
 * Generate prescription based on diagnosis using Gemini API
 */
export async function generatePrescription(
  ...
  try {
    // Use Gemini API to generate prescription
    const prescription = await generatePrescriptionWithGemini(
      severity,
      lesionCounts,
      clinicalMetadata,
      additionalNotes,
...
/**
 * Fallback rule-based prescription generation (used if Gemini API fails)
 */
function generatePrescriptionFallback(
  severity: string,
  lesionCounts: Record<string, number>,
  clinicalMetadata: Record<string, any> = {},
  additionalNotes?: string,
